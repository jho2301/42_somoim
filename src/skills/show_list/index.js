...
app.action(/paginate.*/, async ({ action, body, ack, respond }) => {
  try {
    ...
    await respond({
      ...
      blocks: await createSomoimListBlock(parseInt(action.value, 10), 5, campusNo),
    });
  } catch (err) {
    console.error(err);
  }
...
app.action('show-list', async ({ body, ack }) => {
  await ack();
  await showListMessage({ user_id: body.user.id, channel_id: body.container.channel_id }, { user_id: body.user.id });
});

exports.showListMessage = showListMessage;
